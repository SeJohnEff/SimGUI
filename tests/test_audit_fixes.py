...
class TestCardManagerUncoveredPaths:
    ...
    def test_parse_pysim_output_key_value(self):
        ...
        assert cm.card_info.get("ICCID") == "89494400000016727060"
        assert cm.card_info.get("IMSI") == "99988001000001"

    def test_parse_pysim_output_empty_string(self):
        """_parse_pysim_output leaves card_info unchanged for empty string."""
        from managers.card_manager import CardManager
        cm = CardManager()
        cm.card_info = {}
        cm._parse_pysim_output("")
        assert isinstance(cm.card_info, dict)
    ...
    def test_parse_pysim_output_multiline_value(self):
        """_parse_pysim_output handles lines with colons in value part."""
        from managers.card_manager import CardManager
        cm = CardManager()
        cm.card_info = {}
        # ICCID line is recognized; Ki line has no special handling but no crash
        output = "Ki: AB:CD:EF\nICCID: 89494400000016727060\n"
        cm._parse_pysim_output(output)
        assert "ICCID" in cm.card_info
...
class TestNegativeInputHandling:
    ...
    def test_backup_create_to_nonexistent_dir(self, tmp_path):
        ...
        # create_backup(card_data, filepath) — pass empty card data dict
        try:
            result = bm.create_backup({}, path)
            # Succeeds (creates dirs) or returns None on failure
            assert result == path or result is None
    ...
    def test_validate_adm1_empty_string(self):
        """validate_adm1() with empty string returns None (empty is valid/optional)."""
        from utils.validation import validate_adm1
        result = validate_adm1("")
        # Empty ADM1 is considered valid (optional field) — returns None
        assert result is None
...
class TestIntegrationVerification:
    ...
    def test_find_duplicate_iccids_no_duplicates(self, tmp_path):
        ...
        nsm._profiles = [fake_profile]
        # If the method requires initialisation, skip gracefully
        try:
            result = nsm.find_duplicate_iccids(fake_profile, [r["ICCID"] for r in rows])
    ...
    def test_find_duplicate_iccids_with_duplicates(self, tmp_path):
        ...
        nsm._profiles = [fake_profile]
        try:
            result = nsm.find_duplicate_iccids(fake_profile, [iccid])
