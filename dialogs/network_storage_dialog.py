...
class NetworkStorageDialog(tk.Toplevel):
    ...
    def _test_worker(self, profile):
        """Background worker for connection test."""
        ok, msg = self._ns.test_connection(profile)
        self.after(0, self._test_done, ok, msg)

    def _test_done(self, ok, msg):
        """Update UI with test result (runs on main thread)."""
        self._status_label.configure(text=msg[:80])
        if ok:
            self._status_label.configure(text=f"✓ {msg[:75]}")
        else:
