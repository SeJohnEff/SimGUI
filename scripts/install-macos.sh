...
# NOTE: gialersim Ki/OPc programming no longer goes through pySim.
# SimGUI programs gialersim cards natively (managers/gialersim.py) in GSM class
# with the full algorithm config — pySim's GialerSim class writes keys in the
# wrong class and they never commit. The former dual-ADM (0x0B) VERIFY patch was
# a red herring and has been removed; ref 0x0C with key 84796153 is correct.
# The SPN 'name' handler patch above is retained for any residual pySim use.
